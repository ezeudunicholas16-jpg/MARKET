...
function statusText(context: CommandContext): string {
  ...
  return [
    ...
    `Last Gemini original output word count: ${ai.lastGeminiOriginalOutputWordCount ?? "none"}`,
    `Last Gemini rewrite output word count: ${ai.lastGeminiRewriteOutputWordCount ?? "none"}`,
    `Last final output word count: ${ai.lastFinalOutputWordCount ?? "none"}`,
    `Last quality failure reasons: ${(ai.lastQualityFailureReasons ?? []).join(", ") || "none"}`,
    `Last quality text evaluated source: ${ai.lastQualityTextEvaluatedSource ?? "none"}`,
